...
if(isset($GLOBALS['result']->request_search_details)){
    ...
    <ul class="no-ellipses" data-role="listview" data-filter="true" data-filter-placeholder="Filter.." data-inset="true">
                ...
                if($result_search->request_id != 0){
                    ...
                    <li>
                        <a data-transition="slide" href="index.php?page=view-request&id=<?php echo $result_search->request_id; ?>&ref_page=RequestSearch">
                            ...
                            <?php if($GLOBALS['result']->location_entered == "Y") { ?><p><b>Address: </b><?php if(isset($result_search->house_suffix) && isset($result_search->house_no) && strlen($result_search->house_no) > 0 && strlen($result_search->house_suffix) > 0 && $result_search->house_no != $result_search->house_suffix){ echo base64_decode($result_search->house_suffix); } else{ echo $result_search->house_no; } if(isset($result_search->street_name)){ echo " " .$result_search->street_name; } if(isset($result_search->street_type)){ echo " " .$result_search->street_type; } if(isset($result_search->locality_name)){ echo " " .$result_search->locality_name; } ?></p><?php } ?>
                            <?php if($GLOBALS['result']->facility_entered == "Y") { ?><p><b>Facility: </b><?php echo base64_decode($result_search->facility_name); ?></p><?php } ?>
                            <?php if($GLOBALS['result']->request_description_entered == "Y") { ?><p>Request Description: <b></b><?php echo base64_decode($result_search->request_description); ?></p><?php } ?>
        ...
        else if(count($GLOBALS['result']->request_search_details) == 1){
