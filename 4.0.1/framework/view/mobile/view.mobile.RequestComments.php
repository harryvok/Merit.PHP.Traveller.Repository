        ...
        <ul class="no-ellipses" data-role="listview" data-count-theme="b" data-inset="true">
            ...
            if(isset($GLOBALS['result']->request_remark_details) && count($GLOBALS['result']->request_remark_details) > 1){
                ...
                foreach($GLOBALS['result']->request_remark_details as $result_c_get){
                    $i=$i+1;
            ?>
            <li id="Comment<?php echo $i; ?>ParentObject"><a data-icon="modify" class="edit" id="Comment<?php echo $i; ?>">
                ...
                <p><b>Comment:</b>
                    <div id="Comment<?php echo $i; ?>Label" class="font-normal"><?php if(isset($result_c_get->comment)){ echo base64_decode($result_c_get->comment); } ?></div>
                </p>
            </a></li>
            <li id="Comment<?php echo $i; ?>Edit" style="display:none">
                <p>
                    <?php if($_SESSION['roleSecurity']->maint_comment == "Y"){ ?>
                        ...
                        <a data-role="button" id="Comment<?php echo $i; ?>Submit" data-action="Comment">Save</a><br />
                    <?php } ?>
                    <?php if($_SESSION['roleSecurity']->delete_comment == "Y"){ ?>
                        <a class="delete" id="Comment<?php echo $i; ?>Parent" data-role="button" data-action="Comment" data-mode="DELETE" data-request-id="<?php echo $_GET['id']; ?>" data-note-id="<?php echo $result_c_get->note_id; ?>" data-action-id="<?php echo $result_c_get->action_id; ?>" data-note-class="<?php echo $result_c_get->note_class; ?>"><img src="images/delete-icon.png" width="16" height="16" /> Delete</a>
                        
                    <?php } ?>
                    <?php if($_SESSION['roleSecurity']->delete_comment == "Y" || $_SESSION['roleSecurity']->delete_comment == "Y"){ ?>
            ...
            ?>
            <li id="Comment0ParentObject"><a data-icon="modify" class="edit" id="Comment0">
                <p><b>Level:</b> <?php if($result_c_get->action_id == 0){ echo "Request"; } else { echo "Action"; } ?></p>
                <?php if($result_c_get->action_id != 0 && isset($result_c_get->action_name)){ ?><p><b>Action Required:</b> <?php if(isset($result_c_get->action_name)) echo $result_c_get->action_name; ?></p><?php } ?>
                <p><b>Date</b>: <?php if(strlen($result_c_get->note_datetime) > 0){ echo date('d/m/Y h:i A',strtotime(str_ireplace("T","", $result_c_get->note_datetime))); }  ?></p>
                <p><b>Officer:</b> <?php echo $result_c_get->officer; ?></p>
                <p><b>Type:</b> <?php echo $result_c_get->sub_type; ?></p>
                <p><b>Comment:</b>
                    <div id="Comment0Label" class="font-normal"><?php if(isset($result_c_get->comment)){ echo base64_decode($result_c_get->comment); } ?></div>
                </p>
            </a></li>
