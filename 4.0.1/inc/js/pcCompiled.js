...
// Edit Text
$(document).on(eventName, ".editText", function () {
    var self = this;
    $("#" + $(this).attr("id") + "Label").hide();
    $("#" + $(this).attr("id") + "Text").slideDown("fast");

    $("#" + $(this).attr("id") + "Close").on(eventName, function () {
        $("#" + $(self).attr("id") + "Text").slideUp();
        $("#" + $(self).attr("id") + "Label").slideDown("fast");
    });

    $("#" + $(this).attr("id") + "Submit").on(eventName, function () {
        $("#" + $(self).attr("id") + "Text").slideUp("fast");
        var OldText = $("#" + $(self).attr("id") + "Label").html();
        $("#" + $(self).attr("id") + "Label").html($("#" + $(self).attr("id") + "TextVal").val());
        $("#" + $(self).attr("id") + "Label").slideDown("fast");
        $.ajax({
            url: 'inc/ajax/ajax.modifyRequest.php',
            type: 'post',
            dataType: "json",
            data: { requestID: $("#requestID").val(), id: $(self).attr("id"), text: $("#" + $(self).attr("id") + "TextVal").val() },
            success: function (data) {
                if (data.status == false) {
                    alert("Error modifying. Please contact Merit or try again later.");
                    $("#" + $(self).attr("id") + "Label").html(OldText);
                }
                else {
                    $("#" + $(self).attr("id") + "Text").hide();
                }
            }
        });
    });
...
$(document).ready(function () {
    ...
    $(document).on("keyup", ".tableSearch", function () {
        ...
        if ($(this).val() != "") {
            ...
            if ($("#filterChange").val() != "") {
                $("#" + $(this).attr("id") + "Table td.filterTypeValue:not(:contains-ci('" + $("#filterChange").val() + "'))").parent("tr").hide();
            }
        }
        else {
    ...
    $("#filterChange").on("change", function () {
        if ($(this).val() == "") {
            ...
            $("table tbody>tr").hide();
            $("table td.filterTypeValue:contains-ci('" + $(this).val() + "')").parent("tr").show();
        }
    });
...
$(document).on("click", ".editComment", function(){
    var commentText = $(this).parent().prev().html();
...
$(document).on("click", ".editAttachment", function(){
    ...
    var attachDesc = $(this).parent().prev().prev().last().last().html();
