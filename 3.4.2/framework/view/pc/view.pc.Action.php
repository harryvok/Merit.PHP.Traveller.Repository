...
?>
    <div class="summaryContainer">
      ...
      <span class="summaryColumnTitle">Date Input</span>
        <div class="summaryColumn"><?php if(strlen($GLOBALS['result']['action']->assign_datetime) > 0){ echo date('d/m/Y',strtotime(str_ireplace("00:00:00.000", "", $GLOBALS['result']['action']->assign_datetime))); }?></div>
...
<span class="summaryColumnTitle">Due Date</span>
        <div class="summaryColumn"><?php if(strlen($GLOBALS['result']['action']->due_datetime) > 0 && $GLOBALS['result']['action']->due_datetime != "1970-01-01T00:00:00"){ echo date('d/m/Y h:i A',strtotime(str_ireplace("00:00:00.000", "", $GLOBALS['result']['action']->due_datetime))); }?>
    ...
    <div class="summaryColumn"><?php if(isset($GLOBALS['result']['request']->refer_no)){ echo $GLOBALS['result']['request']->refer_no; } ?></div>
     ...
     <span class="summaryColumnTitle">Request Description</span>
            <div class="summaryColumn">
			  <?php /* Display the description */  if(isset($GLOBALS['result']['request']->request_description)){ echo base64_decode($GLOBALS['result']['request']->request_description); } ?>
            </div>
            <span class="summaryColumnTitle">Request Instructions</span>
            <div class="summaryColumn">
			  <?php /* Display the description */  if(isset($GLOBALS['result']['request']->request_instruction)){ echo base64_decode($GLOBALS['result']['request']->request_instruction); } ?>
            </div>
            </div>
     <div class="float-left">
     <p>&nbsp;</p>
      ...
      <table id="filterTableJobs" class=" sortable" title="" cellspacing="0">
        ...
        elseif(count($GLOBALS['result']['udfs']->udf_details) == 1){
            ...
            <tr class="light_nocur">
              <?php if($udf->udf_type != "C" && $udf->udf_type != "E"){ ?><td><?php echo $udf->udf_name; ?><?php if(isset($udf->udf_mandatory_ind) && $udf->udf_mandatory_ind == "Y" || $udf->udf_mandatory_ind == "I")  echo "<span style='color:red;'>*</span>"; ?></td><?php } ?>
              <td <?php if($udf->udf_type == "C" || $udf->udf_type == "E"){ ?>colspan="2"<?php } ?>><?php if($udf->udf_type == "G" || $udf->udf_type == "B" || $udf->udf_type == "P"){ ?><?php if(isset($udf->udf_data)) {
                                                                                                                                                                                                   if(stristr(str_ireplace("\\", "/", $udf->udf_data), ATTACHMENT_FOLDER)){ ?><a id="A1" class="ViewFile">View</a> <?php }
                                                                                                                                                                                               }
                                                                                                                                                                                                                                                                                                 } elseif($udf->udf_type == "D"){ echo strlen($udf->udf_data) > 0 ? date("d/m/Y", strtotime(str_replace("/","-",$udf->udf_data))) : ""; } elseif($udf->udf_type == "V"){ echo strlen($udf->udf_data) > 0 ? date("d/m/Y h:i A", strtotime(str_replace("/", "-", $udf->udf_data))) : ""; } else{ echo $udf->udf_data; } ?></td>
            </tr>
            <?php
                      ...
                      ?>
                          ...
                          <div class="float-left">
                          		<?php if(isset($udf->udf_data)){
                                            if(stristr(str_ireplace("\\", "/", $udf->udf_data), ATTACHMENT_FOLDER)){ ?><a id="A2" class="ViewFile">View Attachment</a> <?php } else{ echo $udf->udf_data; }
                                        }  ?><br />
                ...
                elseif($udf->udf_type == "B"){
                    $i=$i+1;
                      ?>
                          ...
                          <div class="float-left">
                          		<?php if(isset($udf->udf_data)){
                                            if(stristr(str_ireplace("\\", "/", $udf->udf_data), ATTACHMENT_FOLDER)){ ?><a id="A3" class="ViewFile">View Attachment</a> <?php } else{ echo $udf->udf_data; }
                                        }  ?><br />
