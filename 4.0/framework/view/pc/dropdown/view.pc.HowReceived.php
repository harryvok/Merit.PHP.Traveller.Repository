...
<select name="howReceived" id="howReceived">
    ...
    if(isset($GLOBALS['result']->how_received_details) && count($GLOBALS['result']->how_received_details) > 1){
        foreach($GLOBALS['result']->how_received_details as $priority){ ?>
            <option <?php if(strlen($GLOBALS['default']) > 0){ echo $GLOBALS['default'] == $priority->how_received_code ? "selected='selected'" : ''; } ?> value="<?php echo $priority->how_received_code; ?>"><?php echo $priority->how_received_name; ?></option>
        <?php  
        }
    }
